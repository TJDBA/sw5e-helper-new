...
export class CardRenderer {
  ...
  static buildTargetsContext(targets, state, isGM) {
    ...
    return targets.map((target, index) => {
      ...
      return {
        ...
        // Add these missing fields that the template expects:
  ...
  static buildDamageContext(target, saveOnly, status) {
    ...
    return {
      ...
      damage: target.damage // Keep original damage object
    };
  }
  ...
  static buildSaveContext(target, hasSave) {
    ...
    return {
      ...
      saveResult,
      // Add these for template compatibility:
      save: target.save // Keep original save object
    };
  }
  ...
  static renderHeader(context) {
    const attackInfo = context.attackInfo 
      ? `<span class="info-icon" data-action="show-attack-formula" title="${game.i18n.localize('SW5EHELPER.AttackFormulaTooltip')}">ⓘ</span>`
      : "";
  ...
  static renderControls(context) {
    const quickDamage = !context.hideQuickDamage 
      ? `<a class="icon-btn" data-action="card-quick-damage" title="${game.i18n.localize('SW5EHELPER.QuickDamage')}">⚡</a>`
      : "";

    const modDamage = !context.hideModDamage
      ? `<a class="icon-btn" data-action="card-mod-damage" title="${game.i18n.localize('SW5EHELPER.ModDamage')}">🎲</a>`
      : "";

    const gmToolbar = context.isGM ? `
      <div class="gm-toolbar">
        ${!context.hideRollAllSaves ? `<a class="mini-btn" data-action="gm-roll-all-saves">${game.i18n.localize('SW5EHELPER.RollAllSaves')}</a>` : ""}
        <a class="mini-btn" data-action="gm-apply-all-full">${game.i18n.localize('SW5EHELPER.ApplyAllFull')}</a>
      </div>
    ` : "";

    const expandToggle = `<a class="mini-btn toggle-all-btn" data-action="toggle-all">${context.expandedAll ? game.i18n.localize('SW5EHELPER.CollapseAll') : game.i18n.localize('SW5EHELPER.ExpandAll')}</a>`;
      ...
      <details class="target-row${missingClass}${altClass}" data-target-ref="${target.ref}" ${openAttr}>
        <summary class="summary-row" data-action="toggle-details">
          ...
          <span class="row-actions">
            ...
            ${target.missing ? `<span class="missing">[${game.i18n.localize('SW5EHELPER.Missing')}]</span>` : ""}
          </span>
        </summary>
        ${body}
      </details>
  ...
  static renderSaveLine(target) {
    ...
    const result = target.saveResult 
      ...
      : `<a class="mini-btn" data-action="roll-save" data-target-ref="${target.ref}">${game.i18n.localize('SW5EHELPER.RollSave')}</a>`;

    return `
      <div class="save-line">
        <span>🛡️ ${target.saveAbility || game.i18n.localize('SW5EHELPER.Save')} | DC: ${dcDisplay}</span>
        ${result}
      </div>
    `;
  }
  ...
  static renderDamageLine(target) {
    ...
    return `
      <div class="damage-line">
        <span>💥 ${game.i18n.localize('SW5EHELPER.Damage')}:</span>
  ...
  static renderDamageControls(target) {
    ...
    return `
      <span class="icons">
        <a class="icon-btn" data-action="row-mod-damage" data-target-ref="${target.ref}" title="${game.i18n.localize('SW5EHELPER.ModDamage')}">🎲</a>
        <a class="icon-btn" data-action="apply-full" data-target-ref="${target.ref}" title="${game.i18n.localize('SW5EHELPER.ApplyFull')}">💯</a>
        <a class="icon-btn" data-action="apply-half" data-target-ref="${target.ref}" title="${game.i18n.localize('SW5EHELPER.ApplyHalf')}">½</a>
        <a class="icon-btn" data-action="apply-none" data-target-ref="${target.ref}" title="${game.i18n.localize('SW5EHELPER.ApplyNone')}">Ø</a>
      </span>
    `;
  }
}
